...
export default function BuyerPayments() {
  ...
  const { userProfile, payments, invoices } = useApp();

  if (!userProfile) {
    return <View style={[styles.container, { paddingTop: insets.top }]} />;
  }

  const invoiceIds = new Set(invoices.filter((inv) => inv.buyerId === userProfile.business.id).map((inv) => inv.id));
        ...
        {myPayments.length === 0 ? (
          ...
          myPayments.map((payment) => (
            <View key={payment.id} style={styles.card}>
              ...
              <Text style={styles.meta}>To: {payment.to}</Text>
